...
describe('smoke.api:', function () {
  ...
  describe('setOptions', () => {
    it('works', () => {
      smoke.api.setOptions({ url: smoke.config.get('websocket') });
    });
  });
  ...
  describe('useApiOptions', () => {
    it('works ok with the dev instances', async() => {
      smoke.api.setOptions({ useAppbaseApi: true, url: smoke.config.get('websocketdev') });

      const result = await smoke.api.getContentAsync('yamadapc', 'test-1-2-3-4-5-6-7-9');
      smoke.api.setOptions({ useAppbaseApi: false, url: smoke.config.get('websocket') });
